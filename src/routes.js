...
*/
exports.handleStart = async ({ page }, requestQueue, { location, category }) => {
    if (category === 'forRent') {
        await page.waitForSelector(selectors.RENT_TAB, { timeout: 60000 });
        await page.click(selectors.RENT_TAB);
    }
...
exports.handleProperty = async ({ page, crawler }, { maxItems }) => {
    ...
    const propertyData = await page.evaluate(({ PROPERTY_PRICE, PROPERTY_BEDS, PROPERTY_BATHS, PROPERTY_SQUARE_FOOTAGE,
        PROPERTY_ADDITIONAL_INFO_HEADERS, PROPERTY_ADDITIONAL_INFO_CONTENT, PROPERTY_IMAGE }) => {
        ...
        const preparedPropertyData = {
            ...
            propertyImageUrl: $(PROPERTY_IMAGE).attr('src'),
        };
        for (const [i, header] of extraInfoHeaders.entries()) {
            // Ignore redundant field
            if (header.trim().toLocaleLowerCase() !== 'baths') {
